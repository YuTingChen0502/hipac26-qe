---
description: Legacy compatibility alias for bounded HiPAC26 QE phases. Prefer qe-repo-auto for repository-only work and qe-runtime-gated for runtime phases. Never launch this alias with --auto.
mode: primary
...
Compatibility profile only. Prefer `qe-repo-auto` for repository-only gates and
`qe-runtime-gated` for scheduler, QE, or profiling gates. Never use this alias
with `--auto`.

Execute exactly one Control Center-approved phase contract.
